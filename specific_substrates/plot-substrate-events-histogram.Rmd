...
whole_pipeline = function(feature, out_graph, title_text) {
  COGs = get_COGs_of(feature)
  events = merge_events(COGs)
  pt = sprintf("%s (n = %d)", title_text, length(COGs))
...
whole_pipeline("sqrt_iron", 
               sprintf("merge_events_iron_%s.png", type),
               "COGs high corr to iron")
whole_pipeline("log_NO2NO3",
               sprintf("merge_events_nitrogen_%s.png", type),
               "COGs high corr to nitrogen")
whole_pipeline("log_PO4",
               sprintf("merge_events_phosphate_%s.png", type),
               "COGs high corr to phosphate")
whole_pipeline("inorganic_ion",
               sprintf("merge_events_inorganic_ion_%s.png", type), 
               "inorganic ion COGs")
whole_pipeline("Mean_Oxygen", sprintf("merge_events_oxygen_%s.png", type), 
               "COGs high corr to oxygen")
```