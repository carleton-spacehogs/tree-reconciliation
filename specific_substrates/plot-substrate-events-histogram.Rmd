...
```{r}
all_location_dfs = rbind(get_events("sqrt_iron", "iron", location = TRUE),
                get_events("Fe2+_text_search", "Fe2_textSearch", location = TRUE),
                get_events("Fe3+_text_search", "Fe3_textSearch", location = TRUE),
                get_events("log_NO2NO3", "Nitrogen", location = TRUE),
                get_events("log_PO4", "PO4", location = TRUE),
                get_events("Mean_Oxygen", "Oxygen", location = TRUE),
                get_events("inorganic_ion", "inorganic_ion", location = TRUE))

ggplot(all_location_dfs, aes(x=left.date, y=substrate, color = left.origin, group = substrate)) + # midpoint.date
  geom_jitter(alpha = 0.3, height = 0.3) +
    scale_x_reverse(limits = c(4000, 0),
                    breaks = seq(0, 4000, by = 500)) +
    scale_color_manual(values = c("marine_deep" = "blue",
                                  "marine_deep.marine_shallow" = "steel blue",
                                  "marine_deep.terrestrial" = "yellow",
                                  "marine_shallow" = "light blue",
                                  "marine_shallow.terrestrial" = "orange",
                                  "not_sure" = "white",
                                  "terrestrial" = "red")) +
    labs(x = "Million years ago", y = "") +
    # geom_hline(yintercept = 20.5) +
