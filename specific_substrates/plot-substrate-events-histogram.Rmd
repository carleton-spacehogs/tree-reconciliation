...
make_graph(iron, sprintf("merge_events_iron_%s.png", type), "COGs high corr to iron")
make_graph(NO2NO3, sprintf("merge_events_nitrogen_%s.png", type), "COGs high corr to nitrogen")
make_graph(PO4, sprintf("merge_events_phosphate_%s.png", type), "COGs high corr to phosphate")
make_graph(oxygen, sprintf("merge_events_oxygen_%s.png", type),  "COGs high corr to oxygen")
make_graph(inorganic_ion, sprintf("merge_events_inorganic_ion_%s.png", type),  "inorganic ion COGs")
```

## plotting everything together
```{r}
all_dfs = rbind(iron, NO2NO3, PO4, oxygen, inorganic_ion)
# all_dfs = all_dfs %>% mutate(substrate = fct_relevel(substrate, c("")))

ggplot(all_dfs, aes(x=midpoint.date, y=substrate, color = event, group = substrate)) +
