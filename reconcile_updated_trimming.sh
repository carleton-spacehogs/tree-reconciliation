...
makeTree_and_reconcile() {
	...
	trimal -in $pre_trim -out $trimv1 -gt 0.50 # -automated1
	
	# we excluded genes with more than 20% gaps in the trimmed alignment
	python3 scripts/keep_seq_geq_Xpercent.py $trimv1 $trimv2 50
	rm $trimv1
...
align_makeTree_and_reconcile() {
	...
	if [ ! -f $trimv2 ]; then
		...
		if [ "$num_genes" -gt 1000 ]; then
			./muscle5.1 -super5 -align $gene_seq_file -output $pre_trim
		else
			./muscle5.1 -align $gene_seq_file -output $pre_trim
		fi
	fi
